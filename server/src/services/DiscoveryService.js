...
export class DiscoveryService {
    ...
    async run(interests) {
        ...
        for (const site of suggestedSites) {
            ...
            const result = await this.rssFetcher.fetch(site.url, site.category);
            
            if (result.success && result.items.length > 0) {
                console.log(`[DiscoveryService] OK: ${site.name} は有効なRSSフィードです。 (${result.items.length}件の記事を確認)`);
