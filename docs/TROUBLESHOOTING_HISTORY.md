...
### #26 アクリル背景透過の透過度（視認性）向上対応 (2026-06-14)
- **事象**: WindowsのスナップアシストやFancyZones機能を維持するため `transparent: false` を保っているが、メインウィンドウの背景不透明度（0.7〜0.75）が高すぎて透過感が薄く、背面デスクトップが透けて見えにくい。
- **原因**: UI/CSSの設計において、セマンティックな背景変数 `--surface-base` のアルファ値（不透明度）が高めに設定されていたため。
- **対処**:
    - [index.css](file:///c:/Users/charg/myWorkspace/aegis-ai-hub/src/index.css) 内のテーマ背景変数 `--surface-base` を調整。ライトテーマ時の不透明度を `0.7` から `0.45` へ、ダークテーマ時の不透明度を `0.75` から `0.35` へ引き下げ、アクリル透過をより活かせるように変更。
- **検証**:
    - `npm run build` & `npm run build:electron` によるコンパイル・ビルドの正常動作を確認。
    - `npm run test:e2e -- --project=chromium` を実行し、全12件のE2Eテストがすべて正常にパスすることを確認。
