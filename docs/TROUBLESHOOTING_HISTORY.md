...
---
### #17 Gemini API 利用量可視化機能の実装とE2Eテストの安定化 (2026-05-21)
- **事象**: E2Eテストにおいて「Usage」タブへの遷移や要素の検出がタイムアウト・失敗する問題が発生。
- **原因**:
    1. **環境依存のAPI参照**: `UsageDashboard.tsx` で `window.nexusApi` を直接参照していたため、非Electron環境（Playwrightブラウザ）で統計データの取得に失敗していた。
    2. **初期化ダイアログの干渉**: ブラウザ環境での UI設定取得が `isInitialized: false` で固定されていたため、常に初期化ダイアログが表示され、背後の「Usage」タブへの操作を妨害していた。
    3. **セレクターの曖昧さ**: `has-text` による要素選択が部分一致であったため、複数の要素にマッチし Playwright の strict mode violation が発生していた。
- **対処**:
    - **ブラウザ対応の強化**: `UsageDashboard.tsx` を `src/api/nexusApi.ts` モジュール経由の呼び出しに修正し、自動的に HTTP フォールバックが働くように改善。
    - **ルーティングの拡張**: `NexusRouter.ts` (Fastify) に UI設定 (`ui-settings`) および統計データ (`usage-stats`) の HTTP エンドポイントを追加。
    - **テストの厳格化**: `usage.test.ts` において、API レスポンスを正確にモック化。要素選択を `{ exact: true }` または親コンテナの指定により厳格化し、曖昧さを排除。
    - **クリーンアップ**: テスト終了時のリセット処理を確実に実行するように修正。
